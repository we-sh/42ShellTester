...
*[spec > 42sh > globbing > brace-expansion > list-of-values](..) > 002-simple-test-1*

The purpose of this test is to check that brace expansion is processed when at least one comma `,` is encountered.
### Shell commands that are sent to the standard entry

```bash
./write_all_arguments_on_stdout {${GLOBAL_TOKEN}_A,${GLOBAL_TOKEN}_B}
...
```bash
expected_to match_regex "^${GLOBAL_TOKEN}_A@${GLOBAL_TOKEN}_B@$"
