...
*[spec > 42sh > globbing > brace-expansion > list-of-values](..) > 003-simple-test-2*

The purpose of this test is to check that brace expansion is processed with a list of three patterns.
### Shell commands that are sent to the standard entry

```bash
./write_all_arguments_on_stdout {${GLOBAL_TOKEN}_A,${GLOBAL_TOKEN}_B,${GLOBAL_TOKEN}_C}
...
```bash
expected_to match_regex "^${GLOBAL_TOKEN}_A@${GLOBAL_TOKEN}_B@${GLOBAL_TOKEN}_C@$"
