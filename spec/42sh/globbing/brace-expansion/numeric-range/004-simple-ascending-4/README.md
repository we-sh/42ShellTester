...
*[spec > 42sh > globbing > brace-expansion > numeric-range](..) > 004-simple-ascending-4*
...
```bash
./write_all_arguments_on_stdout {-5..0}${GLOBAL_TOKEN}
...
```bash
expected_to match_regex "^-5${GLOBAL_TOKEN}@-4${GLOBAL_TOKEN}@-3${GLOBAL_TOKEN}@-2${GLOBAL_TOKEN}@-1${GLOBAL_TOKEN}@0${GLOBAL_TOKEN}@$"
