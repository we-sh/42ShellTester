...
*[spec > 42sh > local-variable](..) > 004-existing-variable-in-environment-1*
...
```bash
export "${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_VALUE_OLD"
...
```bash
${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_VALUE_NEW
./display_env
...
```bash
expected_to match_regex "^${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_VALUE_NEW$"
expected_to_not match_regex "^${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_VALUE_OLD$"
