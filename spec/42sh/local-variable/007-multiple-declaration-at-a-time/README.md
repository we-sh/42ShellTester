...
*[spec > 42sh > local-variable](..) > 007-multiple-declaration-at-a-time*

The purpose of this test is to check that declaring multiple variables in the same command line does work.
### Shell commands that are sent to the standard entry

```bash
${GLOBAL_TOKEN}_NAME1=${GLOBAL_TOKEN}_VALUE1 ${GLOBAL_TOKEN}_NAME2=${GLOBAL_TOKEN}_VALUE2 ${GLOBAL_TOKEN}_NAME3=${GLOBAL_TOKEN}_VALUE3
./write_on_stdout_and_stderr $${GLOBAL_TOKEN}_NAME1 $${GLOBAL_TOKEN}_NAME1
./write_on_stdout $${GLOBAL_TOKEN}_NAME2
./write_on_stderr $${GLOBAL_TOKEN}_NAME3
...
```bash
expected_to match_regex "^${GLOBAL_TOKEN}_VALUE1$"
expected_to match_regex "^${GLOBAL_TOKEN}_VALUE2$"
expected_to_not match_regex "^${GLOBAL_TOKEN}_VALUE3$"
...
```bash
expected_to match_regex "^${GLOBAL_TOKEN}_VALUE1$"
expected_to_not match_regex "^${GLOBAL_TOKEN}_VALUE2$"
expected_to match_regex "^${GLOBAL_TOKEN}_VALUE3$"
