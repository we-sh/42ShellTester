...
*[spec > 42sh > local-variable > mixed > redirections](..) > 002-appending*
...
```bash
${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_FILENAME
./write_on_stdout ${GLOBAL_TOKEN}_VALUE1 >> $${GLOBAL_TOKEN}_NAME
./write_on_stdout ${GLOBAL_TOKEN}_VALUE2 >> $${GLOBAL_TOKEN}_NAME
...
```bash
expected_to create_file "${GLOBAL_TOKEN}_FILENAME" matching_regex "^${GLOBAL_TOKEN}_VALUE1$"
expected_to create_file "${GLOBAL_TOKEN}_FILENAME" matching_regex "^${GLOBAL_TOKEN}_VALUE2$"
