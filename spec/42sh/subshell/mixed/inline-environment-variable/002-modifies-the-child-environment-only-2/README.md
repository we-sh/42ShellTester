# 002-modifies-the-child-environment-only-2

*[spec > 42sh > subshell > mixed > inline-environment-variable](..) > 002-modifies-the-child-environment-only-2*
...
```bash
( (${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_VALUE ./display_env) ; ./display_env) ; ./display_env
...
```bash
expected_to match_regex "^${GLOBAL_TOKEN}_NAME=${GLOBAL_TOKEN}_VALUE$" once
